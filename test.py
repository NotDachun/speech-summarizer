...
def summarize_string(input_text, title):
  sentence_count = input_text.count('.')
  parameters = {"text": input_text, "sentences_number": sentence_count//10, "title": title, "language": "en"}
...
def summarize_text(filename, title):
  ...
  parameters = {"text": text, "sentences_number": sentence_count//5, "title": title, "language": "en"}
  return call_api("summarize", parameters)
